...
export async function generateAITitle(
  ...
  try {
    ...
    const Summarizer = (window as unknown as { Summarizer: { create: (options: unknown) => unknown } }).Summarizer;

    // Log first-time use (model download might occur)
    console.log('[AI Title Generator] Initializing AI model (first-time use may download Gemini Nano)...');
    ...
    const result = await (summarizer as { summarize: (text: string) => Promise<string> }).summarize(truncatedContent);
