...
/**
 * Get translated message (with fallback for content script context)
 * @param key i18n message key
 * @param substitutions Optional substitutions
 * @returns Translated message or key if translation unavailable
 */
function getMessage(key: string, substitutions?: string | number | (string | number)[]): string {
  if (typeof chrome !== 'undefined' && chrome.i18n && chrome.i18n.getMessage) {
    return (
      chrome.i18n.getMessage(key, substitutions as string | (string | number)[] | undefined) || key
    )
  }
  return key
}
...
function extractMetadata(): BlockSource {
  ...
  if (favicon && favicon.length > 0 && !favicon.startsWith('http')) {
...
async function executeClip(): Promise<void> {
  ...
  if (!selectedHTML || isEmptyContent(selectedHTML)) {
    showNotification(
      getMessage('content_noContentSelected'),
      getMessage('content_pleaseSelectText'),
    )
    return
  }
  ...
  if (contentLength > 50000) {
    const shouldContinue = confirm(
      getMessage('content_largeContentWarning', [Math.round(contentLength / 1000)]),
    )
    if (!shouldContinue) {
      return
    }
  ...
  if (contentLength > 5000) {
    showNotification(getMessage('common_loading'), getMessage('content_processingLargeContent'))
  }
...
async function clipContent(): Promise<void> {
  ...
  if (clippingQueue.length >= MAX_QUEUE_SIZE) {
    ...
    showNotification(getMessage('content_queued'), getMessage('content_tooManyRequests'))
    return
  }
...
async function confirmSelectorCapture(candidate: HTMLElement): Promise<void> {
  ...
  if (!text.trim()) {
    showNotification(
      'No text to capture',
      'The selected area has no visible text. Choose another area.',
    )
    return
  }
  ...
  try {
    ...
    if (response?.type === 'CLIP_SUCCESS') {
      ...
      const error =
        ...
        'Failed to save visual selection'
      showNotification('✗ Clipping failed', error)
    }
  } catch (error) {
    showNotification(
      '✗ Clipping failed',
      error instanceof Error ? error.message : 'Failed to save visual selection',
    )
  } finally {
    exitSelectorMode()
  }
 ...
 */
function sendMessage<T = unknown>(type: MessageType, data?: T): Promise<MessagePayload | null> {
